        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-3">
              <ShoppingBag size={40} />
              <div className="font-['Barlow_Condensed'] tracking-widest uppercase text-sm">Your cart is empty</div>
              <button
                onClick={onClose}
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-3 border border-border p-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-['Barlow_Condensed'] font-700 uppercase text-sm leading-tight truncate">{item.name}</div>
                  <div className="text-primary font-['Barlow_Condensed'] font-900">${item.price.toFixed(2)}</div>
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-sm font-['Barlow_Condensed'] font-700 w-4 text-center">{item.qty}</span>
                    <button
        ...
        {items.length > 0 && (
          <div className="px-5 py-5 border-t border-border">
            <div className="flex justify-between items-center mb-4">
              <span className="font-['Barlow_Condensed'] text-sm tracking-widest uppercase text-muted-foreground">TOTAL</span>
              <span className="font-['Barlow_Condensed'] text-2xl font-900 text-primary">${total.toFixed(2)}</span>
            </div>
            <button className="w-full bg-primary text-white py-3 font-['Barlow_Condensed'] text-lg font-700 tracking-widest uppercase hover:bg-primary/90 transition-colors">
              CHECKOUT →
            </button>
            <button
              onClick={onClose}
